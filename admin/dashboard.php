...
// Proses perubahan tema
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['theme_name'])) {
    $theme_name = mysqli_real_escape_string($conn, $_POST['theme_name']);
    
    // Update atau insert tema
    $query = "UPDATE settings SET theme_name='$theme_name' ORDER BY id DESC LIMIT 1";
    if (!mysqli_query($conn, $query)) {
        // Jika tidak ada record, insert baru
        $query = "INSERT INTO settings (theme_name) VALUES ('$theme_name')";
...
<body>
    ...
    <div class="content">
        ...
        <!-- Theme Selector -->
        <div class="card theme-selector mb-4">
            <div class="card-header">
                <h5 class="mb-0"><i class="bi bi-palette me-2"></i>Pilih Tema Website</h5>
            </div>
            <div class="card-body p-0">
                <form method="post" action="">
                    <div class="theme-option <?php echo $current_theme == 'theme1' ? 'active' : ''; ?>" onclick="selectTheme('theme1')">
                        <div class="theme-preview theme-1">
                            <i class="bi bi-water"></i>
                        </div>
                        <div class="theme-info">
                            <h6>Tema Blue Ocean</h6>
                            <small>Desain klasik dengan warna biru yang profesional</small>
                        </div>
                        <input type="radio" name="theme_name" value="theme1" <?php echo $current_theme == 'theme1' ? 'checked' : ''; ?> style="display:none;">
                    </div>
                    <div class="theme-option <?php echo $current_theme == 'theme2' ? 'active' : ''; ?>" onclick="selectTheme('theme2')">
                        <div class="theme-preview theme-2">
                            <i class="bi bi-stars"></i>
                        </div>
                        <div class="theme-info">
                            <h6>Tema Purple Dream</h6>
                            <small>Desain split layout dengan warna ungu yang elegan</small>
                        </div>
                        <input type="radio" name="theme_name" value="theme2" <?php echo $current_theme == 'theme2' ? 'checked' : ''; ?> style="display:none;">
                    </div>
                    <div class="theme-option <?php echo $current_theme == 'theme3' ? 'active' : ''; ?>" onclick="selectTheme('theme3')">
                        <div class="theme-preview theme-3">
                            <i class="bi bi-tree"></i>
                        </div>
                        <div class="theme-info">
                            <h6>Tema Green Nature</h6>
                            <small>Desain modern dengan warna hijau yang segar</small>
                        </div>
                        <input type="radio" name="theme_name" value="theme3" <?php echo $current_theme == 'theme3' ? 'checked' : ''; ?> style="display:none;">
                    </div>
                    <div class="p-3 border-top">
                        <button type="submit" class="btn btn-primary">
                            <i class="bi bi-check-circle me-2"></i>Terapkan Tema
                        </button>
                        <a href="../index.php" target="_blank" class="btn btn-outline-secondary ms-2">
